...
def train(cfg, local_rank, distributed, logger):
    ...
    #? 시작을 뜻함, 학습 시작을 로그로 기록합니다.

    feature_extractor = build_feature_extractor(cfg)
    #? 'build_feature_extractor'에서 featuer_extractor을 뽑아냄
    #? 따라서 어떤것이 뽑아 내는지 확인이 필요
    ...
    batch_size = cfg.SOLVER.BATCH_SIZE  # default: 8
    if distributed:
        pg1 = torch.distributed.new_group(range(torch.distributed.get_world_size()))
        batch_size = int(cfg.SOLVER.BATCH_SIZE / torch.distributed.get_world_size())
        if not cfg.MODEL.FREEZE_BN:
            # if don't freeze BN, then replace BN with SyncBatchNorm
            # for self-supervised learning, use SyncBatchNorm
    ...
    if cfg.resume:
        ...
        if "optimizer_fea" in checkpoint:
            ...
            optimizer_fea.load(checkpoint['optimizer_fea'])
        if "optimizer_cls" in checkpoint:
            logger.info("Loading optimizer_cls from {}".format(cfg.resume))
            optimizer_cls.load(checkpoint['optimizer_cls'])
    ...
    for i, (src_input, src_label, _) in enumerate(train_loader):
        ...
        if cfg.SOLVER.LAMBDA_LOV > 0:
            ...
            loss = ce_criterion(pred, src_label) + cfg.SOLVER.LAMBDA_LOV * loss_lov
...
def main():
    ...
    # ? -cfg를 통해서 경로를 지정함, 이것이 yaml을 불러 옴
    ...
    # ? 임의의 추가 설정을 명령줄에서 지정할 수 있게 하는 옵션


    # ! python3 -m torch.distributed.launch --nproc_per_node=$NGPUS 
    # ! train_src.py -cfg configs/samsung/deeplabv2_r101_src.yaml 
    # ! SOLVER.LAMBDA_LOV 0.75 OUTPUT_DIR results/gta5/r101_g2c_src SOLVER.BATCH_SIZE 8
    # ! 위의 명령어는 `run_proca_resnet101_gta5.sh`의 명령어에서 시작한 명령어이다.
    # 'torch.distributed.launch'는 pytorch에서 지원하는 분산 학습을 지원하기 위한 유틸리티이다. 
    # '-cfg configs/gta5/deeplabv2_r101_src.yaml'은 yaml의 경로 이다. 
    ...
    logger = setup_logger("SourceOnly", output_dir, args.local_rank)
    logger.info("Using {} GPUs".format(num_gpus))
    # ? 사용되는 GPU의 개수를 로그로 기록합니다.

    logger.info(args)
    #? 명령 줄에서 전달된 인자들을 로그로 기록합니다. 
    # ? 이를 통해 프로그램이 어떤 설정으로 실행되었는지 추후에 확인할 수 있습니다.

    logger.info("Loaded configuration file {}".format(args.config_file))
    # ? 사용된 설정 파일의 경로를 로그로 기록합니다.

    logger.info("Running with config:\n{}".format(cfg))
    # ? 실제로 사용되는 설정 내용을 로그로 기록합니다. 
    # ? 이 설정은 명령 줄 인자와 설정 파일에서 가져온 정보를 병합한 결과입니다.
